...
/*	public function actionListorderlocations($customer)
	...
	public function actionListparentlocations()
	{
		...
		$_output = '<optgroup label="Main Folders">';
		foreach($parent_locations as $l_parent)
		{
			$parent_code = (!empty($l_parent->parent_code)) ? $l_parent->parent_code . " - " : "";
			$_output .=	'<option value="m_'.$l_parent->id.'">'.$parent_code . $l_parent->parent_name.'</option>';
		}			
		$_output .=	'</optgroup>';	
		//
		foreach(LocationClassment::find()->select('parent_id')->where(['customer_id'=>$customers])->innerJoin('lv_locations', '`lv_locations`.`id` = `lv_locations_classments`.`location_id`')->groupBy('location_id')->distinct()->all() as $l_classment)
		{
			$parent_name = LocationParent::findOne($l_classment->parent_id)->parent_name;
			$_output .=	'<optgroup label="'.$parent_name.'">';
			foreach(Location::find()->innerJoin('lv_locations_classments', '`lv_locations_classments`.`location_id` = `lv_locations`.`id`')->where(['customer_id'=>$customers, 'parent_id'=>$l_classment->parent_id])->groupBy('location_id')->all() as $location)
			{
				$l_output = "";
				if(!empty($location->storenum))
					$l_output .= "Store#: " . $location->storenum . " - ";
				if(!empty($location->storename))
					$l_output .= $location->storename  . ' - '; 
				//
				$l_output .= $location->address . " " . $location->address2 . " " . $location->city . " " . $location->state . " " . $location->zipcode;									
				$_output .=	'<option value="p_'.$location->id.'">'.$l_output.'</option>';
			}
			$_output .=	'</optgroup>';
		}
		//
	...
	public function actionUploadorderfile()
	{
		...
		$picture_name = $this->ordergeneratefilename($name, $new_path);
		...
		if($data['isComplete']){
			$files = $data['data'];
			$media = new Medias;
			$media->filename = $picture_name.".".$extension;
			$media->path = "orders/";
			$media->type = 4;
			$media->save();
			//
	...
	function ordergeneratefilename($name, $path)
	{
...
//			ORDER BY companyname LIMIT 30";
	...
	*/
	...
	public function actionListrepresentativelocations($customer)
	{
		$locations = Location::find()->where(['customer_id'=>$customer])->all();
	...
	public function actionGetinventorypartnumbers($modelid)
	{
		...
		$cust_partnums = Partnumber::find()->select('*')->where(['model'=>$model->id])->distinct()->all();
                ...
                $content = !empty($model->aei) ? "AEI #: " . $model->aei . "<br/>":"";
		$content .= !empty($model->manpartnum) ? "Mfr#: " . $model->manpartnum . "<br/>":"";
		$content .= !empty($model->frupartnum) ? "FRU #: " . $model->frupartnum:"";
	...
	public function actionGetorderqtydetails($idorder)
	{
		$order = Order::findOne($idorder);
		
		$content = "";
		
		$items = Item::find()->where(['ordernumber'=>$idorder])->all();
		//var_dump($items);
		...
		foreach($items as $item)
		{
			...
			$serial = $item->serial;
			if(!empty($serial))
				$newline = "$name <br/> <li style=\"margin-left:20px;list-style-type: square;\">$serial";
			$tagnum = $item->tagnum;
			if(!empty($tagnum) && $tagnum!='' && $tagnum!='undefined')
				$newline .= " ($tagnum)";
			$newline .= "</li>";
			$content .= $newline;
		}
	...
	public function actionGetlocationindexdetails($idlocation)
	{
		$items = Item::find()->where(['location'=>$idlocation])->groupBy('model')->all();
		...
		foreach($items as $item)
		{
			$_model = Models::findOne($item->model);
			$manufacturer = Manufacturer::findOne($_model->manufacturer);	
			$count_model = Item::find()->where(['location'=>$idlocation, 'model'=>$_model->id])->count();
			$name = $manufacturer->name . ' ' . $_model->descrip;
	...
	public function actionGetcustomer($query){
		...
		foreach($datas as $data){
			$returnData[] = array('id' => $data['id'], 'name' => $data['companyname']);
		}
...
/*	
	...
	public function actionGetorderindexstatus($idorder)
	{
		$statuscolors = array('red'=>[1,2,3,4,5], 'yellow'=>[6,7,8,9,10,11], 'green'=>[12,13,14,15,16]);
		
		$model = Order::findOne($idorder);
		//
		/*
		$modelitems = Item::find()->innerJoin('lv_models', '`lv_models`.`id` = `lv_items`.`model`')
								->innerJoin('lv_manufacturers', '`lv_manufacturers`.`id` = `lv_models`.`manufacturer`')
								->where(['ordernumber'=>$model->id])
								->orderBy('status, name, descrip')
								->groupBy('model')
								->all();	
		*/
		$sql = "SELECT lv_items.model FROM lv_items WHERE ordernumber=:order GROUP BY model ORDER BY status";	
		
		$modelitems = Yii::$app->db->createCommand($sql)
								->bindValue(':order', $model->id)
								->queryAll();
							
		//$modelitems = Item::find()->where(['ordernumber'=>$model->id])->orderBy('status')->groupBy('model')->all();	
		
		$content = "";
		
		$_totalitems = Item::find()->where(['ordernumber'=>$model->id])->count();
		
		foreach($modelitems as $modelitem)
		{		
			$totalitems = Item::find()->where(['ordernumber'=>$model->id, 'model'=>$modelitem['model']])->count();
			...
			foreach($statuses as $status)
			{
				...
				if(in_array($status->status, $statuscolors['red']))
					$_status_style = "color: #d43f3a;";
				else if(in_array($status->status, $statuscolors['yellow']))
					$_status_style = "color: #f0ad4e;";
				else if(in_array($status->status, $statuscolors['green']))
					$_status_style = "color: #26B99A;";
				
				$statusitems = Item::find()->where(['ordernumber'=>$model->id, 'model'=>$modelitem['model'], 'status'=>$status->status])->count();
				...
				{
					if(count($statuses) > 1)
						$_content .= "<li style=\"margin-left:20px;list-style-type: square;\">Qty: $statusitems - ";
					$_content .= "<span style=\"$_status_style\">($statuspercentage% " . Item::$status[$status->status] . ")</span><br/>";
					if(count($statuses) > 1)
						$_content .= "</li>";
				}
			}
				...
				$items = Item::find()->where(['ordernumber'=>$idorder, 'model'=>$modelitem['model']])->all();
				...
				{						
					foreach($items as $item)
					{
						$serial = $item->serial;
						if(!empty($serial))
							$newline = "<li style=\"margin-left:20px;list-style-type: square;\">$serial";
						$tagnum = $item->tagnum;
						if(!empty($tagnum) && $tagnum!=''&&$tagnum!='undefined')
							$newline .= " ($tagnum)";
						$__pre_content .= $newline . "</li>";
					}	
				}				
			$content .= $_pre_content . $_content . $__pre_content;
		}	
	...
	public function actionGetordermailstatus($idorder)
	{
		$mailreports = SalesorderMail::find()->where(['orderid'=>$idorder])->groupBy('email')->all();
		$output = "";
		//
		foreach($mailreports as $mailreport)
		{
			$dates = \yii\helpers\ArrayHelper::getColumn(SalesorderMail::find()->where(['orderid'=>$idorder, 'email'=>$mailreport->email])->all(), 'date_sent');
			$sender_dates = array();
			foreach($dates as $date)
			{
				$sender_dates[] = date('m/d/Y', strtotime($date->date_sent)); 
			}
			$output .= "This order was emailed to " . $mailreport->email . " on " . implode(',', $sender_dates) . "<br/>";
		}
		
		$_retArray = array('success' => true, 'html' => $output);
