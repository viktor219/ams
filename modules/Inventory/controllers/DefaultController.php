...
class DefaultController extends Controller
{
    ...
    public function actionIndex()
    {
       ...
       // $searchModel = new InventorySearch();
        ...
        $queryCart = Wccart::find()->innerJoin('lv_models', '`lv_wccart`.`modelid` = `lv_models`.`id`')->where(['userid'=>Yii::$app->user->id, 'used'=>0]);
        ...
        return $this->render('index', [
           ...
           'dataCartProvider' => $dataCartProvider,
        ]);
    }
    ...
    public function actionAdditemtoorder()
    {
    	...
    	if(Wccart::find()->where(['userid'=>Yii::$app->user->id, 'modelid'=>$id, 'used'=>0])->one() === null)
    	{
	    	$wccart = new Wccart;
	    	$wccart->userid = Yii::$app->user->id;
	    	$wccart->modelid = $id;
	    	$wccart->used = 0;
	    	$wccart->save();
    	}
    	...
    	$query = Wccart::find()->innerJoin('lv_models', '`lv_wccart`.`modelid` = `lv_models`.`id`')->where(['userid'=>Yii::$app->user->id, 'used'=>0]);
    	...
    	$html = $this->renderPartial('_loaded_model_warehouse', [
    ...
    public function actionRemoveitemtoorder()
    {
    	$_post = Yii::$app->request->get();
    	...
    	Wccart::find()->where(['userid'=>Yii::$app->user->id, 'modelid'=>$id, 'used'=>0])->one()->delete();
    ...
    public function actionSaveworderquantity()
    {
    	$_post = Yii::$app->request->get();
    	...
    	$find_model_cart = Wccart::find()->where(['userid'=>Yii::$app->user->id, 'modelid'=>$id, 'used'=>0])->one();
    	
    	$find_model_cart->quantity = $_post['quantity'];
    	
    	$find_model_cart->save();
    ...
    public function actionResetitemtoorder()
    {
    	$_post = Yii::$app->request->get();
    	 
    	$_retArray = array('success' => FALSE, 'html' => '');

    	$cart = Wccart::deleteAll(['userid'=>Yii::$app->user->id, 'used'=>0]);
...
//			$count = count($command->queryAll());
	...
	public function actionCreatewarehouseorder()
	{
		...
		$shipment->save();		
		//
		foreach ($data['models'] as $_modelid)
		{
			$_find_model = Models::findOne($_modelid);
			$_data_model = $_modelid;
			//
			if(!$_find_model->assembly)
			{
				$value = $data['quantity'][$_modelid][0];
			...
			//
			$cart_model = Wccart::find()->where(['userid'=>Yii::$app->user->id, 'modelid'=>$_modelid, 'used'=>0])->one();
			$cart_model->used = 1;
			$cart_model->save();
		}
		//
		if($successorder === true){
			...
			$_message = json_encode($model->errors) . '<br/>' . json_encode($item->errors) . '<br/>' . json_encode($order->errors);
		...
		foreach($data['itemserial'] as $_key=>$_value)
		{
			...
			{		
				...
				if($location->id != $current_location)
				{
					...
					$item->location = $data['rma_location'];
					//
