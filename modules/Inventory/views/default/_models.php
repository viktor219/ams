<?php 
	...
	use yii\widgets\Pjax;
        ...
        $_templatesaccess = "{view}{update}{delete}{reorder}{merge}{transfer}";
...
//                                                                ->andwhere('status IN ('.array_search('In Stock', Item::$status).', '.array_search('Ready to ship', Item::$status).')')
                                        ...
                                        {
                                                $sum_in_stock =  Item::find()->where(['model'=>$model['id'], 'status'=>4])->count();
                                                if(Yii::$app->user->identity->usertype == User::REPRESENTATIVE || Yii::$app->user->identity->usertype==User::TYPE_CUSTOMER)
                                                	$sum_in_stock =  Item::find()->where(['customer'=>$customers, 'model'=>$model['id'], 'status'=>4])->count();
...
//                                                $sum_readytoship = $model['readytoship_qty'];
	                    ...
	                    [
	                        ...
	                        'contentOptions' => ['style' => $_defaultwidth, 'class' => 'action-buttons'],
	                        //'visible' => (Yii::$app->user->identity->usertype!==User::REPRESENTATIVE && Yii::$app->user->identity->usertype!==User::TYPE_CUSTOMER),
	                        'controller' => 'inventory',
	                        'buttons' => [
	                        	'warehouseorder' => function ($url, $model, $key) { 
	                        		$options = [
	                        			'title' => 'Create A Warehouse Order',
	                        			'class' => 'btn btn-info btn-sm warehouseorder',
	                        			//'onClick' => 'openWarehouseModal('.$model['id'].');'
	                        			'onClick' => 'addItemToWarehouseOrder('.$model['id'].');',
										'disabled' => 'true'
	                        		];
	                        		$url = 'javascript:;';
