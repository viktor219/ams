...
class DefaultController extends Controller
{
	...
	public function actionSendmail()
	{
		if (Yii::$app->request->isAjax) {
			...
			$model = $this->findModel($_post['purchaseId']);
			...
			$body = $this->renderPartial('_pordermailtemplate', ['model' => $model, 'vendor'=>$vendor, 'content'=>nl2br($_post['body'])]); 
			...
			if(filter_var($tomail, FILTER_VALIDATE_EMAIL) !== false)
			{
				...
				$success = true;
				//track mail sent.
				$mail = new PurchaseMail;
				$mail->purchaseid = $_post['purchaseId'];
				$mail->email = $tomail;
				$mail->save();
			}
			else
	...
	private function generateLocalOrder($id)
	{
    	...
    	$cssContent = "
    		...
    		#shipping_methods td, #shipping_methods th, #products th, #products td {text-align:center;}
    ...
    public function actionLoad(){
		if(Yii::$app->user->identity->usertype != 1)
		{
			$query0 = Item::find()
				...
				->where(['`lv_salesorders`.ordertype'=>1, '`lv_items`.status'=>array_search('Requested', Item::$status), 'purchaseordernumber'=>NULL, 'lv_items.deleted'=>0]);
		} else {
			$customers = ArrayHelper::getColumn(UserHasCustomer::find()->where(['userid'=>Yii::$app->user->id])->asArray()->all(), 'customerid');
			$query0 = Item::find()
				->innerJoin('lv_salesorders', '`lv_salesorders`.`id` = `lv_items`.`ordernumber`')
				->where(['`lv_salesorders`.ordertype'=>1, '`lv_items`.status'=>array_search('Requested', Item::$status), 'purchaseordernumber'=>NULL])
				->andWhere(['`lv_salesorders`.`customer_id`'=>$customers, 'lv_items.deleted'=>0]);						
		}
		
		if(Yii::$app->user->identity->usertype==User::TYPE_TECHNICIAN)
		{
			$query0->innerJoin('lv_orderlog', '`lv_orderlog`.`orderid` = `lv_salesorders`.`id`')
					->innerJoin('lv_users', '`lv_users`.`id` = `lv_orderlog`.`userid`')
					->andWhere(['lv_users.department'=>Yii::$app->user->identity->department]);
		}
		
		$query0->groupBy('model, ordernumber');	
        ...
        if(Yii::$app->user->identity->usertype==User::TYPE_TECHNICIAN)
        {
        	$query->innerJoin('lv_users', '`lv_users`.`id` = `lv_purchases`.`user_id`')
        	->andWhere(['lv_users.department'=>Yii::$app->user->identity->department]);        	
        }        
    ...
    public function actionLoadtype()
    {
    	...
    	if($type == 'service')
    	{
    		if(Yii::$app->user->identity->usertype != 1)
    		{
    			$query = Item::find()
    			->innerJoin('lv_salesorders', '`lv_salesorders`.`id` = `lv_items`.`ordernumber`')
    			->where(['`lv_salesorders`.ordertype'=>2, '`lv_items`.status'=>array_search('Requested', Item::$status), 'purchaseordernumber'=>NULL])
    	...
    	else if($type == 'integration')
    	{
	    	if(Yii::$app->user->identity->usertype != 1)
	    	{
	    		$query = Item::find()
	    		->innerJoin('lv_salesorders', '`lv_salesorders`.`id` = `lv_items`.`ordernumber`')
	    		->where(['`lv_salesorders`.ordertype'=>3, '`lv_items`.status'=>array_search('Requested', Item::$status), 'purchaseordernumber'=>NULL])
	    		->andWhere('lv_items.deleted = 0')
	    		->groupBy('model');
	    	} else {
	    		$customers = ArrayHelper::getColumn(UserHasCustomer::find()->where(['userid'=>Yii::$app->user->id])->asArray()->all(), 'customerid');
    	...
    	else if($type == 'warehouse')
    	{
	    	if(Yii::$app->user->identity->usertype != 1)
	    	{
	    		$query = Item::find()
	    		->innerJoin('lv_salesorders', '`lv_salesorders`.`id` = `lv_items`.`ordernumber`')
	    		->where(['`lv_salesorders`.ordertype'=>4, '`lv_items`.status'=>array_search('Requested', Item::$status), 'purchaseordernumber'=>NULL])
	    		->andWhere('lv_items.deleted  = 0')
	    		->groupBy('model');
	    	} else {
	    		$customers = ArrayHelper::getColumn(UserHasCustomer::find()->where(['userid'=>Yii::$app->user->id])->asArray()->all(), 'customerid');
    ...
    public function actionCreate($request=null)
    {
    	...
    	//generate models
		PHelper::generatePurchasingJson();
    	//
    	$item_requested = null;
