...
<?= GridView::widget([
        ...
        'columns' => [
			...
			[
				...
				'template'=>'{createpo} {create} {update} {delete}',
				'contentOptions' => ['style' => 'width:180px;', 'class' => 'action-buttons'],
				'visibleButtons' => [
					'createpo' => function ($model, $key, $index) use ($type){
						return $type == "service" ? true : false;
					},
					'create' => function ($model, $key, $index) use ($type){
						return $type != "service" ? true : false;
					},					
				],
				'buttons' => [
					'createpo' => function ($url, $model, $key) {
