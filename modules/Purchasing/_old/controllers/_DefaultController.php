				...
				'rules' => [
					[
						'actions' => [
							'index','create', 'update', 'view', 'delete',
							'search', 'sendmail', 'sendmailform',
							'generate', 'receive', 'savereceiveqty', 'receiveqty', 'delitems', 'delpurchase'						
						],
						'allow' => true,
						// Allow few users
						'roles' => [
    ...
    public function actionIndex()
    {
		...
		if(Yii::$app->user->identity->usertype != 1)
		{
			$query1 = Item::find()
				...
				//->andFilterWhere(['owner_id'=>Yii::$app->user->id])
				->groupBy('model');
		} else {
			$customers = ArrayHelper::getColumn(UserHasCustomer::find()->where(['userid'=>Yii::$app->user->id])->asArray()->all(), 'customerid');
			$query1 = Item::find()
        ...
        $deletedIncomingPurchases = Purchase::find()->innerJoin('lv_items', '`lv_items`.`purchaseordernumber` = `lv_purchases`.`id`')
                    ->andWhere('lv_purchases.deleted = 1')->groupBy('lv_purchases.id')->count();
    	//    	
        ...
        return $this->render('index', [
        	'__requesteddataProvider' => $dataProvider0,
            '__purchasedataProvider' => $dataProvider,
            '__ipurchasedataProvider' => $dataProvider3,
        	'__integrationdataProvider' => $dataProvider1,
        	'__warehousedataProvider' => $dataProvider2,
        	'__servicedataProvider' => $dataProvider4,
                'deletedIncomingPurchases' => $deletedIncomingPurchases,
                'deletedItemsRequested' => $deletedItemsRequested
        ]);
    }
	...
	public function actionSendmail()
	{
		if (Yii::$app->request->isAjax) {
			...
			*/	
			...
			//if($sent)
				$_retArray = array('success' => true, 'message' => 'Mail is sent succefully');
			echo json_encode($_retArray);
			exit();				
		} else {
