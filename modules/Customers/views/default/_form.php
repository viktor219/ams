...
$_secondaryshipping_method = new ShipmentMethod;
if(!$customer->isNewRecord)
{
	...
	if($customer->defaultshippingchoice==0)
	{
		...
		$_defaultshipping_method = ShipmentMethod::findOne($_setting->shipping_method);
	}
	else if($customer->defaultshippingchoice==1)
	{
		$_setting = CustomerSetting::find()->where(['customerid'=>$customer->id])->one(); 
		$_defaultshipping_method = ShipmentMethod::findOne($_setting->default_shipping_method);
	}
	else if($customer->defaultshippingchoice==2)
	{
		$_setting = CustomerSetting::find()->where(['customerid'=>$customer->id])->one();
		$_defaultshipping_method = ShipmentMethod::findOne($_setting->secondary_shipping_method);
	}
}
...
<?= $this->render("_modals/_addlocation", ['_shipping_method'=>$_defaultshipping_method]);?>
						...
						<div class="row row-margin well well-sm">
							<div class="form-group">	
								<div class="col-sm-6">
									...
									<select class="receiving_select2_single form-control inputs" name="defaultreceivinglocation" <?php if($customer->isNewRecord) :?> disabled <?php endif;?>>
