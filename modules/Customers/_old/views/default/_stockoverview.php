<?php 
	...
	$this->params['breadcrumbs'][] = $customer->companyname . ' ' . $this->title;	
	
	$items_customer = Item::find()->where(['customer'=>$customer->id])->groupBy('customer')->all();
?>
<style>
...
<?= $this->render("_modals/_editcategorymodal");?>
	...
	<div class="panel-heading">
		<div class="row vertical-align">
			...
			<div class="col-md-10 vcenter text-right"> 
				<select class="parent_child_location" id="viewByLocation">
					<option value="">View By Location</option>
					<?php foreach($items_customer as $item) :?>
						<?php 
						...
						?>			
						<option <?php if($_location->id === $location->id) :?>selected<?php endif;?> value="<?php echo $location->id;?>"><?php echo $output;?></option>		
					<?php endforeach;?>
				</select>
				<?= Html::a('<span class="glyphicon glyphicon-globe"></span> Manage Locations', ['/customers/locations', 'customer'=>$customer->id], ['class' => 'btn btn-success', 'style' => 'margin-left: 5px;border-radius:4px;']) ?>
				<?= Html::a('<span class="glyphicon glyphicon-equalizer"></span> View Orders', ['/orders/index', 'customer'=>$customer->id], ['class' => 'btn btn-success', 'style' => 'margin-left: 5px;border-radius:4px;']) ?>
				<?= Html::a('<span class="glyphicon glyphicon-export"></span> View Shipments', ['/shipments/index', 'customer'=>$customer->id], ['class' => 'btn btn-success', 'style' => 'margin-left: 5px;border-radius:4px;']) ?>
				<?= Html::a('<span class="glyphicon glyphicon-arrow-down"></span> Expand All', 'javascript:;', ['class' => 'btn btn-success', 'style' => 'margin-left: 5px;border-radius:4px;', 'onClick'=>'showAllModels();']) ?>
			</div>                
		</div>
	</div>
					...
					<div class="x_content">
						<?php foreach($categories as $category): ?>
							<div class="row" style="padding: 2px; padding-left: 10px; font-size: 14px; background: #73879C; color: #FFF; border-radius:5px; margin-bottom: 10px;">
								<div class="col-md-5 vcenter" style="line-height: 30px;">
									<span class="glyphicon glyphicon-lock"></span> <b><?php echo $category->categoryname;?></b>
								</div>
								<div class="col-md-7 vcenter text-right" style="margin-top:3px;"> 
							...
							<div id="loaded-content-category-<?php echo $category->id;?>" style="display: none;">
								...
								<?php 
									...
									$sql .= (isset($_location)) ? "
													SUM(lv_items.status='". array_search('In Stock', Item::$status)."' OR lv_items.status='".array_search('In Progress', Item::$status)."' OR (lv_items.status='".array_search('Shipped', Item::$status)."' AND location='".$_location->id."')) as total,
											": "
									...
									$sql .= "
													...
													lv_models.descrip,
													CONCAT(lv_manufacturers.name, ' ', lv_models.descrip) AS description,
													lv_models.aei,
													lv_models.image_id,
											...
											'columns' => [	
												...
												[
													...
													'value' => function($model) use ($customer){
														...
														return '<div style="line-height: 40px;font-weight:bold;">' . $model['description'] . '</div>';
													}
												],
												[
													'format' => 'raw',
