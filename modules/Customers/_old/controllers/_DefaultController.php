...
class DefaultController extends Controller {
    	...
    	return [
	    	'access' => [
		    	...
		    	'only' => [
							...
							'showallprojects', 'locations', 'locationview', 'locationdelete', 'ownstockpage', 'statusdetails', 'serialdetails'				
						],
		    	'rules' => [
			    	[
				    	'actions' => [
	...
	public function actionLoadinventorystats()
	{
		...
		//                                });
		$sql = 'select (((sum(t.total) - (sum(t.test))) / sum(t.total))* 100 ) as percent, abs((((sum(t.total) - (sum(t.test))) / sum(t.total))* 100 )) as absolute_percent, t.customer, sum(t.test) as count from ((SELECT count(*) as total, customer, "0" as test FROM `lv_items` WHERE status = :status_in_stock and DATE_FORMAT(lastupdated,"%Y-%m-%d") < :last_monday group by customer) UNION (SELECT "0" as total, customer, count(*) as test FROM `lv_items` WHERE status = :status_in_stock group by customer))t group by t.customer order by absolute_percent desc limit :start_limit, :offset';
...
//		->bindValue(':status_ready_ship', array_search('Ready to ship', Item::$status))
	...
	public function actionLoadrmamodelslocation()
	{
    	...
    	//if (Yii::$app->request->isAjax) {
			...
			$sql = "SELECT GROUP_CONCAT(DISTINCT(lv_items.model)) as rows_model,
							GROUP_CONCAT(lv_items.id) as rows_id, 
							GROUP_CONCAT(lv_items.serial) as rows_serial,
							GROUP_CONCAT(lv_models.serialized) as rows_serialized,
							GROUP_CONCAT(lv_items.tagnum) as rows_tagnums
						FROM lv_items 
			...
			$_location_detail = LocationDetail::find()->where(['locationid'=>$location->id])->one();
    		...
    		$_retArray = array('success' => true, 'html' => $html, 'count' => count($items));
    		echo json_encode($_retArray);
    		exit();	
		//}
	}
	...
	public function actionStatusdivisionsdetails($type, $customer, $model, $parent)
	{
		$customer = Customer::findOne($customer);
		
		$model = Models::findOne($model);
		
		$query = Item::find()->innerJoin('lv_locations_classments', '`lv_locations_classments`.`location_id` = `lv_items`.`location`')
							->innerJoin('lv_locations', '`lv_locations`.`id` = `lv_items`.`location`')
							->where(['customer'=>$customer->id, 'model'=>$model->id, 'parent_id'=>$parent]);
		
		if($type==1)
			$query = $query->andWhere(['storenum'=>'DIV']);
		else if($type==2)
			$query = $query->andWhere(['<>', 'storenum', 'DIV']);
		else 
			$query = $query->andWhere(['confirmed'=>1]);
        ...
        return $this->render('_statusdivisionsdetails', [
			'dataProvider' => $dataProvider,
			'customer' => $customer,
			'model' => $model,
			'parent' => $parent
	...
	public function actionLoadeditcategoryform()
	{
    	...
    	if (Yii::$app->request->isAjax) {
    		...
    		$html = $this->renderAjax('_loadeditcategoryform', [
    					'model' => $model,
    					'category' => $category,
