    ...
    */	
							...
							for($i=0; $i<$_assemblyQty;$i++){
								...
								if($model->save()){
									...
									$errors .= $model->errors; 
                                        ...
                                        $recentActivity->pk = $_modelid;
                                        $recentActivity->customer_id = $model->customer;
                                        $recentActivity->user_id = Yii::$app->user->id;
                                        $recentActivity->created_at = date('Y-m-d H:i:s');
                                        $recentActivity->type = array_search('Items Received', Recentactivity::$type);
                                        $recentActivity->itemscount = $value;
    ...
    public function actionOserialform()
    {
    	...
    	//if (Yii::$app->request->isAjax) {
    		...
    		$_currentquantity = (!empty($session['currentquantity'])) ? $session['currentquantity'] : 0;
    		...
    		// $_retArray = array('success' => true, 'html' => $html, 'itemserialized'=>Item::find()->where(['customer'=>$customer->id, 'model'=>$model, 'status'=>array_search('In Stock', Item::$status)])->andWhere(['not', ['serial'=>null]])->count(), 'itemserializedornot'=>$quantity);
    		$_retArray = array('success' => true, 'html' => $html, 'itemserialized'=>$_currentquantity, 'itemserializedornot'=>$quantity);
    		echo json_encode($_retArray);
    		exit();  
    	//}  	
    }
    
    private function validateserial($serial, $model, $customer)
    {
    	...
    	*/
    	...
    	if(!empty($checkit) && !$hasstartedwithcheckit){
    		$_message = "Error 100 : Serial number must be start with $checkit";
    	}
    	/**
    	 ...
    	 */
    		...
    		$_message = "Error 200 : Serial number must have $charactercount characters without '-'";
    	}
    	 
    	else if(!$customer->trackincomingserials && $_find_serial!==null)
    		$_message = "Error 300 : Serial Number ($serial) already exists!";
    
    	else if($customer->trackincomingserials && $_find_serial!==null && $_find_serial->status != array_search('In Stock', Item::$status) && $_find_serial->customer==$customer->id) {
    		$_findcustomer = Customer::findOne($_find_serial->customer);
    		$_message = "Error 400 : Serial Number ($serial) is already <b>" . Item::$status[$_find_serial->status] . "</b> for <b>" . $_findcustomer->companyname . "</b>. It is has been modified to \"In Stock\" status.";
    	} else if($customer->trackincomingserials && $_find_serial!==null /*&& $_find_serial->status == array_search('In Stock', Item::$status)*/ && $_find_serial->customer!=$customer->id) {
    		$_findcustomer = Customer::findOne($_find_serial->customer);
    		$_message = "Error 500 : Serial Number ($serial) is already <b>" . Item::$status[$_find_serial->status] . "</b> for <b>" . $_findcustomer->companyname . "</b>.";
    	}
    
    	else if($customer->trackincomingserials && $_find_in_inventory_serial!==null)
    		$_message = "Error 600 : Serial Number ($serial) already exists in " . $customer->companyname . " inventory!";
    ...
    public function actionSaveserial()
    {
    	...
    	$_validateserial = $this->validateserial($serial, $model, $customer);
    	$_old_location = 0;
    	//$_post = array('serial'=>$serial, 'currentmodel'=>4, 'quantity'=>4, 'customerId'=>1);
    	//if (Yii::$app->request->isAjax) {
    	$_retArray = array('success' => FALSE, 'html' => '');
    	if(empty($_post['skipValidation']))
    	{
    		if (!isset($serial) || !isset($model) || !isset($quantity) || !isset($customer)) {
    			$_retArray = array('error' => true, 'html' => 'Something is wrong! Plese try again!');
    			echo json_encode($_retArray);
    			exit();
    		} else if(!empty($_validateserial)) {
    			if(strpos($_validateserial, ':') !== false)
    			{
    				$_validateserial = explode(':', $_validateserial);
    				$_code = str_replace('Error', '', $_validateserial[0]);
    				$_code = (int) trim($_code);
    				$_error = $_validateserial[1];
    			}
    			//
    			if($_code != 400)
    			{
    				$_retArray = array('error' => true, 'html' => $_error, 'code'=>$_code);
    				echo json_encode($_retArray);
    				exit();
    			}
    		}
    	...
    	$current_item = Item::find()->where(['serial'=>$serial])->one();
    
    	if(empty($current_item))
    		$current_item = new Item;
    	else {
    		if($location != $current_item->location)
    			$_old_location = $current_item->location;
    	}
    	$current_item->owner_id = Yii::$app->user->id;
    	$current_item->customer = $customer;
    	$current_item->ordernumber = null;
    	...
    	if($current_item->save()){
    		...
    		if($_old_location)
    		{
    			...
    			$itemlog->itemid = $current_item->id;
    			$itemlog->locationid = $_old_location;
    			$itemlog->save();
    		}
    		//echo $current_quantity;
    		//echo $quantity;
    		$done =  ($current_quantity===$quantity) ? true : false;
    		$_retArray = array('success' => true, 'message' => 'Serial number is succefully saved!', 'code'=>$_code, 'current_quantity' => $current_quantity);
    		if($_code == 400)
    			$_retArray['alert_msg'] = $_error;
    		if($done)
    			$_retArray['done'] = $done;
    	}
    	else
