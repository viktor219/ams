...
class DefaultController extends Controller
{
        ...
        return [
	    	'access' => [
	    		...
	    		'only' => [	
							...
							'validateserial', 'saveserial', 'addcustomer', 'labeltest'
						],
		    	'rules' => [
		    		[
		    			...
		    			'actions' => [
							...
							'sendmailform', 'updatemodel', 'serialform', 'refurbishform',
							'picklistreadyform', 'picklistreadymodel', 'refurbish', 
							'validateserial', 'saveserial', 'addcustomer', 'getdeleted', 'revert', 'rdelete', 'labeltest'			
						],
		    			'allow' => true,
		    			// Allow few users
		    			'roles' => [
	...
	public function actionSendreturnedlabelmail()
	{
		...
		$body = $this->renderPartial('_labelordermailtemplate', ['model'=>$model, 'customer'=>$customer, 'content'=>$_labelcontent]);
    ...
    public function actionLabeltest()
	{
		//if (Yii::$app->request->isAjax) {
			...
			//
			if(ReturnedlabelFile::find()->where(['orderid'=>$order->id])->one() === null)
			{
				//echo 'ok!';
			...
			//var_dump($label_files);
			
			$html = $this->renderPartial('_labelreturn', ['model'=>$order, 'files'=>$label_files]);
			
			$_retArray = array('success' => true, 'html' => $html, 'id' => $id, 'title' => 'SO# ' . $name);			
	...
	private function generateLabelOrder($id, $package, $extension = ".png")
	{
    	$model = $this->findModel($id);
		...
		*/ 
		...
		file_put_contents(Yii::getAlias('@webroot') . '/' . $path, base64_decode($package['label_img']));
    	...
    	$cssContent = "
    		.kv-heading-1, th, td {font-size:18px}
    		@page { 
			  background: url('/testing/live/$path') no-repeat center center fixed;  
		...
		$targetfile = self::_temp_PDF_PATH . $package['pkg_trk_num'] . '.pdf';
