...
$trackingnumbers = ArrayHelper::getColumn(Shipment::find()->where(['orderid'=>$model->id])->asArray()->all(), 'trackingnumber');

$_shipping_company_name = $shipping_company->name;
?>
<section style="overflow-y:auto;" id="generated_pdf">
	<div id="body" style="height: 955px;">
	...
	<table id="shipping_methods">
		...
		<tr>
			<td><?php echo (!empty($_shipping_company_name)) ? $_shipping_company_name : '-';?></td>
			<td><?php echo (!empty($__shipping_method)) ? str_replace($_shipping_company_name, '', $__shipping_method) : '-';?></td>
			<td>-</td>
			<td><?php echo $model->customer_po;?></td>
			<td><?php echo $model->enduser_po;?></td>
			<td><?php echo implode('-', $trackingnumbers);?></td>
		</tr>	
	</table>
