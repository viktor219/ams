...
$instock = Item::find()->where(['status'=>4, 'customer'=>$customer])->count();
$picked = Item::find()->where(['ordernumber'=>$model->ordernumber, 'status' => array_search('Picked', Item::$status)])->count();
...
<?= $this->render("_modals/_serials", ['order'=>$order]);?>	
