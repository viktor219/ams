<?php 
	...
	$_cleaninghasoption = Item::find()->innerJoin('lv_item_has_option', '`lv_item_has_option`.`itemid` = `lv_items`.`model`')
	...
	$count_preowneditems = Item::find()->innerJoin('lv_models', '`lv_models`.`id` = `lv_items`.`model`')->where(['ordernumber'=>$model->id, 'preowneditems'=>1])->count();
	
	$count_cleaningitems = Item::find()->where(['ordernumber'=>$model->id, 'status'=>array_search('Cleaned', Item::$status)])->count();
	
	if(!$_cleaninghasoption && !$count_preowneditems || $count_cleaningitems)
		...
		<div class="panel-heading">
			<div class="row vertical-align"> 
				...
				<div class="col-md-4 vcenter text-right">
					...
					<?= Html::a('<span class="glyphicon glyphicon-store"></span> Ready To Ship All', ['/inprogress/turnonship', 'id'=>$model->id], ['class' => 'btn btn-success', 'onClick' => "return confirm('Are you sure that all items are ready for ship ?');"]) ?>									
				</div>
			</div>
		</div>
		<div class="panel-body">
                                        	<div role="tabpanel" class="tab-pane fade active in" id="inventoryhome" aria-labelledby="home-tab">
                                				...
                                				<div class="col-md-1" style="padding:0;">
                                					<?= Html::a('<span class="glyphicon glyphicon-store"></span> All Cleaned', ['/inprogress/markallcleaned', 'id'=>$model->id], ['class' => 'btn btn-success']);?>	
                                				</div>
												<div class="col-md-3">
