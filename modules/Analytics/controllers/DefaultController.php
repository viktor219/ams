...
//use app\vendor\BP\BackgroundProcess\BackgroundProcess;
    ...
    public function behaviors() {
        return [
            'access' => [
                ...
                'only' => ['index', 'reportsettings', 'setoption', 'getserviceopenreport', 'getserviceclosereport', 'getreallocationreport', 'getdivisionreport', 'testfull'],
                'rules' => [
                    [
                        'actions' => ['index', 'setoption', 'reportsettings', 'getserviceopenreport', 'getserviceclosereport', 'getreallocationreport', 'getdivisionreport', 'testfull'],
                        'allow' => true,
                        'roles' => [
                            User::TYPE_ADMIN,
...
//            $user = \app\models\Users::findOne(Yii::$app->user->id);            
            $query = Item::find()
                    ->innerJoin('lv_models', '`lv_models`.`id` = `lv_items`.`model`')
                    ->innerJoin('lv_salesorders', '`lv_items`.`ordernumber` = `lv_salesorders`.`id`')
                    ->innerJoin('lv_salesorders_ws', '`lv_salesorders_ws`.`service_id` = `lv_salesorders`.`id`')
                    ->where(['customer' => $customer->id, 'lv_salesorders.ordertype' => 2, 'lv_salesorders.deleted'=>0])
                    ->andWhere('( status >= ' . array_search('Shipped', Item::$status) . ' AND status <= 21 ) OR status = '. array_search('Received', Item::$status))
            		->orderBy('lv_salesorders.created_at DESC');
//            if($user->division_id){
...
//            $user = \app\models\Users::findOne(Yii::$app->user->id);
            ...
            $query = Item::find()
                    ...
                    ->innerJoin('lv_salesorders', '`lv_items`.`ordernumber` = `lv_salesorders`.`id`')
                    ->where(['customer' => $customer->id, 'lv_salesorders.ordertype' => 2, 'lv_salesorders.deleted'=>0])
                    ->andWhere('status = ' . array_search('In Transit', Item::$status) . ' OR status = '.array_search('Awaiting Return', Item::$status))
                    ->orderBy('lv_salesorders.created_at DESC');
//            if($user->division_id){
//                $reportSetting = \app\models\ReportsSettings::find()
//                        ->innerJoin('lv_reports_types', 'lv_reports_types.id = lv_reports_settings.report_type_id')
...
//        $user = \app\models\Users::findOne(Yii::$app->user->id);
        ...
        $query = Item::find()
                ...
                ->orderBy('lv_itemslog.created_at DESC');
//        if($user->division_id){
//            $reportSetting = \app\models\ReportsSettings::find()
//                    ->innerJoin('lv_reports_types', 'lv_reports_types.id = lv_reports_settings.report_type_id')
...
//        }
        if (isset($_post['submit'])) {
        	            $report_types = [];
        	//            \app\models\ReportsSettings::deleteAll(['userid' => Yii::$app->user->id]);
        	            $success = true;        	
            foreach ($_post['report_option_id'] as $type => $report_opts) {
                ...
                if (!$report_setting_model->save()) {
                            ...
                            if(count($reportTypes)){
                	                $reportTypes = implode(",", $reportTypes);
                	                $reportSettingsDel = \app\models\ReportsSettings::find()
                	                        ->where(['userid' => Yii::$app->user->id])
                	                        ->andWhere('report_type_id NOT IN ('.$reportTypes.')')
                	                        ->all();
                	            } else {
                		                $reportSettingsDel = \app\models\ReportsSettings::find()->where(['userid' => Yii::$app->user->id])->all();
                		            }
                		            foreach($reportSettingsDel as $reportSettingDel){
                			                    $reportSettingDel->delete();
                			            }
                			            if($success){
                					            ...
                					            return $this->redirect(['/analytics/reportsettings']);
            } 
        //}
...
//                $output .= $location->storename . ' - ';
            ...
            $dateCreated = (!empty($order->created_at) && $order->created_at != "0000-00-00 00:00:00") ? date('m/d/y h:iA', strtotime($order->created_at)) : "-";
            ...
            $division = (isset($locationClassment) && $locationClassment != NULL) ? LocationParent::findOne($locationClassment->parent_id)->parent_code : 'Uncategorized';
            $phone = $location->phone;
            $openDays = '-';
            if((!empty($order->created_at) && $order->created_at != "0000-00-00 00:00:00")){
                $datediff = (strtotime(date('Y-m-d'))) - (strtotime($order->created_at));
                $openDays = abs(ceil($datediff / (60 * 60 * 24)));
            }
            $serviceOrder = \app\models\SalesorderWs::find()->where(['service_id' => $model['ordernumber']])->one();
            if($serviceOrder != NULL){
                $outgoingShipment = \app\models\Shipment::find()->where(['orderid' => $serviceOrder->warehouse_id])->one();
            }
            $excelDatas[] = [
                ...
                'outgoingtracking' => isset($outgoingShipment->master_trackingnumber)?$outgoingShipment->master_trackingnumber:'',
                'tagnum' => $model['tagnum'],
                'origin' => $address,
        ...
        $objPHPExcel->getActiveSheet()->setTitle($fileName)
                ...
                ->setCellValue('E1', 'Incoming Tracking')
                ->setCellValue('F1', 'Outgoing Tracking')
                ->setCellValue('G1', 'Tagnumber')
                ->setCellValue('H1', 'Store Number')
        ...
        $objPHPExcel->getActiveSheet()->setTitle($fileName)
                ...
                ->setCellValue('M1', 'Days Open');            
        }
        $objPHPExcel->getActiveSheet()->getStyle('E1')->applyFromArray($centerAlign);
        $objPHPExcel->getActiveSheet()->getStyle('G1')->applyFromArray($centerAlign);
        $objPHPExcel->getActiveSheet()->getStyle('H1')->applyFromArray($centerAlign);
        $objPHPExcel->getActiveSheet()->getStyle('J1')->applyFromArray($centerAlign);
        $objPHPExcel->getActiveSheet()->getStyle('A1:N1')->getFont()->setBold(true);
        $row = 2;
        foreach ($excelDatas as $excelData) {
            $open_days = ($excelData['open_days'] == 1) ? '1 Day': $excelData['open_days'].' Days';
            ...
            ->getStyle('D' . $row)->applyFromArray($centerAlign);
            if(!$isClose){
                $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, $excelData['returntracking']);
                $objPHPExcel->getActiveSheet()->setCellValue('F' . $row, $excelData['outgoingtracking']);
                $objPHPExcel->getActiveSheet()->setCellValue('G' . $row, $excelData['tagnum'])
                ->getStyle('F' . $row)->applyFromArray($centerAlign);
                $objPHPExcel->getActiveSheet()->setCellValue('H' . $row, $excelData['store_number'])
                ->getStyle('G' . $row)->applyFromArray($centerAlign);
                $objPHPExcel->getActiveSheet()->setCellValue('I' . $row, $excelData['origin']);
                $objPHPExcel->getActiveSheet()->setCellValue('J' . $row, $excelData['division'])
                ->getStyle('I' . $row)->applyFromArray($centerAlign);
                $objPHPExcel->getActiveSheet()->setCellValue('K' . $row, $excelData['phone']);
                $objPHPExcel->getActiveSheet()->setCellValue('L' . $row, $excelData['created_at']);
                $objPHPExcel->getActiveSheet()->setCellValue('M' . $row, $excelData['created_by']);
                $objPHPExcel->getActiveSheet()->setCellValue('N' . $row, $open_days);                
            } else {
                $objPHPExcel->getActiveSheet()->setCellValue('E' . $row, $excelData['returntracking']);
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('M' . $row, $open_days);                
            }
            $row++;
        }
...
//        }
    ...
    private function _genPdfServiceReport($isClose, $divisionId) {
        ...
        $query->orderBy('lv_salesorders.created_at DESC');
        $models = $query->all();
        $pdfDatas = array();
        foreach ($models as $model) {
            $_model = Models::findOne($model->model);
            ...
            $pdfDatas[] = [
                ...
                'created_at' => $dateCreated,
                'created_by' => $createdBy,
                'open_days' => ($openDays == 1) ? '1 Day': $openDays. ' Days'
            ];
        }
        return $pdfDatas;
    }
