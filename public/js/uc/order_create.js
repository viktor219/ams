...
$("#selectLocation").change(function() {
	...
	if(locationid.length > 0)
	{
		...
		$.getJSON(url, function(data){
			...
			if(data.s2!="")
				$('#shippingcompany').val(data.s2).trigger("change");
			if(data.s3!="")
				loadShippingMethods($('#add-order-form #shippingcompany'), $("#add-order-form #shippingmethod"), data.s3);
			//
			if(data.s1!="" && data.s2!="" && data.s3!="")
			{
...
//$(function () {
	...
	$(document).on("click touchstart", "#switch-shipping-detail-tab [data-switch-set]", function() {
		...
		//clear old entries
		$('#accountnumber').val("");
		$('#shippingcompany').prop('selectedIndex', 0).trigger("change");
		$("#shippingmethod").val("").trigger("change");
		//alert(value);
	...
	$('#autocomplete-customer').typeahead({
		onSelect: function(item) {
			...
			var url = jsBaseUrl+"/ajaxrequest/getshippingchoice?customer=" + customerid;
			$.getJSON(url, function(data){
				//alert(data.choice);
				$('#switch-shipping-detail-tab button').each(function(){
					if(parseInt($(this).data('switch-value'))==data.choice)
						$(this).addClass('active');
					//
					if(parseInt(data.choice)==0)
					{
						var url = jsBaseUrl+"/ajaxrequest/getshippingassetdetails";
						$.getJSON(url, function(data){
							$('#accountnumber').val(data.s1);
					...
					else if(parseInt(data.choice)==1)
					{
						var url = jsBaseUrl+"/ajaxrequest/getshippingmaindetails?customer=" + customerid;
						$.getJSON(url, function(data){
							$('#accountnumber').val(data.s1);
					...
					else if(parseInt(data.choice)==2)
					{
						var url = jsBaseUrl+"/ajaxrequest/getshippingotherdetails?customer=" + customerid;
						$.getJSON(url, function(data){
							$('#accountnumber').val(data.s1);
	...
	if($('#autocompletevalitem_'+rowid).val().length === 0){
		...
		$.ajax({
			...
			if (data.success) {
				...
				$('#entry' + rowid + ' .qty-group').append('<div class="help-block">Exceeds Stock!</div>'); // add the actual error message under our input					
			}
		});	
	}
});	
...
$(document).ready(function () {
	...
	//
	//tab key press on customer field 
	$(".select_ttl").on('keydown', '.input_fn', function(e) { 
		var p = $(this);
		var row = p[0].id.split('_')[1];
		...
		if (keyCode == 9) { 
			//e.preventDefault(); 
			$("#item_s-"+row).focus(); 
			$('#item_s-'+row).select2("open");
		} 
	});	
	$('[id^="item_s-"]').next(".select2").find(".select2-selection").focus(function() {
		$(this).select2("open");
	});
	$(".desc-group").on('keydown', '.select2-search__field', function(e) { 
...
$(function () {
    $(document).on('change', '.select_ttl', function() {
        $('#item-count-order').html('Total : $' + checkOrderAmount(false));
        //alert('changed');
    });
    ...
    $('#btnAdd').click(function () {
        var num = parseInt($('.clonedInput').last().attr('id').replace('entry', '')); // last id of "duplicatable" input fields we currently have
        newNum  = new Number(num + 1);      // the numeric ID of the new input field being added
        ...
        // enable the "remove" button
        $('#btnDel, .btnDel').attr('disabled', false);
    ...
    $('#btnDel').click(function () {
        ...
        // remove the last element
        $('.clonedInput').last().slideUp('fast', function () {
            $(this).remove();
            // if only one element remains, disable the "remove" button
        ...
        // enable the "add" button
        $('#item-count-order').html('Total : $' + checkOrderAmount(true));
    ...
    $(document).on('click', '.btnDel', function () {
        ...
        var clonedInputId = $(this).closest('.clonedInput').attr('id');

        // how many "duplicatable" input fields we currently have
        $('#' + clonedInputId).slideUp('fast', function () {
            $(this).remove();
            // if only one element remains, disable the "remove" button
            if ( $('.clonedInput').length === 1) {
                $('#btnDel, .btnDel').attr('disabled', true);
            }
            // enable the "add" button
            $('#btnAdd').attr('disabled', false).prop('value', "add section");
        });
